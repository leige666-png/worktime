 ...
 * 
 * 登录流程：
 * 1. 首次登录：输入MIS号+姓名+密码 → 注册 → 第一个用户自动管理员，后续为member
 * 2. 后续登录：MIS号+密码 → 进入系统
 * 3. 数据存储在 GitHub 远程仓库，所有用户共享
 */
...
export async function registerUser(mis: string, name: string, password: string): Promise<User> {
  const existing = await userDB.getByMis(mis.trim());
  if (existing) {
...
export async function signInWithPassword(mis: string, password: string): Promise<User> {
  const user = await userDB.getByMis(mis.trim());
  if (!user) {
